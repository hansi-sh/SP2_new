...
void WinScene::Init() //defines what shader to use
{
	...
	meshList[GEO_WINT] = MeshBuilder::GenerateQuad("WinScreent", Color(0, 0, 1), 40.0f, 30.0f, 0.0f);
	meshList[GEO_WINT]->textureID = LoadTGA("Image//wintransparent.tga");

	meshList[GEO_SELECTQUAD] = MeshBuilder::GenerateQuad("selectquad", Color(0, 0.65, 0), 12.15f, 5.0f, 0.0f);
}
...
void WinScene::RenderWin()
{
	...
	modelStack.PushMatrix();
	modelStack.Translate(f_TRestart, 11.2f, 0.1f);
	modelStack.Translate(0, 0, 0);
	RenderMesh(meshList[GEO_SELECTQUAD], false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(f_TExit, -1.0f, 0.1f);
	modelStack.Translate(0, 0, 0);
	RenderMesh(meshList[GEO_SELECTQUAD], false);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	modelStack.Translate(0, 20, 1);
	RenderMesh(meshList[GEO_WINT], false);
	modelStack.PopMatrix();
