...
void CutScene::Init() //defines what shader to use
{
	...
	//Loading Screen
	meshList[GEO_LOADING] = MeshBuilder::GenerateQuad("loading", Color(0, 0, 0), 15, 15, 0);
...
void CutScene::Render()
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		modelStack.Translate(0, 0.1f, 15.f - (i * 14));
	...
	if (b_movemodel = true)
	{
		//<--Lego Model-->
		modelStack.PushMatrix();
		modelStack.Translate(-0.2f, 6, -39);
		modelStack.Translate(0, 0, f_translatemodel);
...
static const float SKYBOXSIZE = 150.f;
...
void CutScene::DrawHUD(Mesh* mesh, bool enableLight, float size, float x, float y)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
