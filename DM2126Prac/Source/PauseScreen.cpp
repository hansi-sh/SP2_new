...
float PauseScene::lastX = 0.0f;
float PauseScene::lastY = 0.0f;
Camera2 PauseScene::camera = Camera2();
...
void PauseScene::Init() //defines what shader to use
{
	//Background color
	...
	meshList[GEO_PAUSE] = MeshBuilder::GenerateQuad("PauseScreen", Color(0, 0, 1), 40.0f, 30.0f, 0.0f);
...
void PauseScene::RenderPause()
{
	...
	modelStack.PushMatrix();
	RenderTextOnScreen(meshList[GEO_TEXT], "Resume", Color(1, 1, 1), 4, 31.2f, 28.5f);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderTextOnScreen(meshList[GEO_TEXT], "Restart", Color(1, 1, 1), 3.6f, 30.3f, 18);
	modelStack.PopMatrix();

	modelStack.PushMatrix();
	RenderTextOnScreen(meshList[GEO_TEXT], "ExitGame", Color(1, 1, 1), 3, 30.5f, 7.5f);
	modelStack.PopMatrix();
}

void PauseScene::RenderMesh(Mesh *mesh, bool enableLight)
{
...
void PauseScene::RenderButton(int geo_circle, int geo_cylinder)
{
	//<----Button circle---->
...
void PauseScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void PauseScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
...
void PauseScene::DrawPause(Mesh* mesh, bool enableLight, float size, float x, float y)
{
	glDisable(GL_DEPTH_TEST);
	Mtx44 ortho;
	ortho.SetToOrtho(0, 80, 0, 60, -10, 10); //size of screen UI
